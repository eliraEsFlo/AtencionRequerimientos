...
(function($) {
    ...
    $.fn.orgChart = function(options) 
    {
        ...
        return this.each(function() {
            ...
            if (opts.levels > -1) {
                $this.find("ul").andSelf()
                    .filter(function() {return $chartSource.parents("ul").length + 1 > opts.levels;}).remove();
            }
    ...
    function buildNode($node, $appendTo, level, index, opts) {
        ...
        if ($childNodes.length > 0) {
            if (opts.depth == -1 || level+1 < opts.depth) {
                ...
                var $list = $("<ul class='stack'>").append($stackNodes).addClass("level"+level)
